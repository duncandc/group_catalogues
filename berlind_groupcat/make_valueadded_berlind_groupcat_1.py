...
#Yale University
#Description: Read in hdf5 berlind groupcat catalogues and add some matched/derived values
#this works on stellar mass limited samples!
...
def main():
    ...
    for group in group_IDs: #run through each group and identify central galaxy
        ...
        central = np.where((data['GROUP_ID']==group) & (data['MSTAR']==largest_mass))[0]
        central = central[0] #should only be one of these, so make it the central
